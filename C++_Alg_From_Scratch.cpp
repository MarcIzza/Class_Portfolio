...
void print_logistic_regression(vector<string>survived, vector<string>sex, int numObservations)
{
	...
	cout << "\n\nCount Survived: " << Totalsurvived;
	cout << "\nCount dead: " << Totaldied << "\n";
	...
	//Now beginning on Logistic regression
...
void calcu_raw_prob(int pclass, int sex, int age, double lh_pclass[2][3], double lh_sex[2][2], double apriori[2], double  age_var[2], double  age_mean[2])
{
	...
	double num_s = lh_pclass[0][pclass] * lh_sex[0][sex] * apriori[0] * calc_age_lh(age, age_mean[1], age_var[1]);

	double num_p = lh_pclass[1][pclass] * lh_sex[1][sex] * apriori[1] * calc_age_lh(age, age_mean[0], age_var[0]);

	double denominator = lh_pclass[0][pclass] * lh_sex[0][sex] * calc_age_lh(age, age_mean[1], age_var[1]) * apriori[0] + lh_pclass[1][pclass] * lh_sex[1][sex] * calc_age_lh(age, age_mean[0], age_var[0]) * apriori[1];
...
void print_NaivieBayes(vector<string>num, vector<double>pclass, vector<string>survived, vector<string>sex, vector<double>age, int numObservations)
{
	...
	cout << "\t Class 1" << "\tClass 2" << "\tClass 3" << endl;
	cout << "Survived: " << lh_pclass[0][0] << " " << lh_pclass[0][1] << " " << lh_pclass[0][2] << "\n\n";
	cout << "Died: " << lh_pclass[1][0] << " " << lh_pclass[1][1] << " " << lh_pclass[1][2] << "\n\n";
	...
	cout << "Survived: " << lh_sex[0][0] << " " << lh_sex[0][1] << "\n\n";
	cout << "Died: " << lh_sex[1][0] << " " << lh_sex[1][1] << "\n\n";
	...
	cout << "\n**I've applied everything to the first 5 test observations for Raw Probabilities**";
...
int main(int argc, char**)
{
	...
	print_NaivieBayes(num, pclass, survived, sex, age, numObservations);


	print_logistic_regression(survived, sex, numObservations);
